...
__global__ void sobelKernel (unsigned char* s, unsigned char* t, unsigned height, unsigned width, unsigned channels) {
    ...
    for (y = blockIdx.x*blockDim.x+threadIdx.x; y < height; y+=1024) {
        for (x = 0; x < width; ++x) {
            for (i = 0; i < MASK_N; ++i) {
...
int main(int argc, char** argv) {
    ...
    unsigned char* host_s1 = NULL;
    cudaMalloc(&host_s1, sizeof(char) * (channels*height*width));
    cudaMemcpy(host_s1, host_s, sizeof(char)* (channels*height*width), cudaMemcpyHostToDevice);

    unsigned char* host_t = NULL;
    cudaMalloc(&host_t, sizeof(char) * (channels*height*width));
    sobelKernel<<<16,64>>>(host_s1, host_t, height, width, channels);

    unsigned char* host_result_t = (unsigned char*) malloc(height * width * channels * sizeof(unsigned char));
    cudaMemcpy(host_result_t, host_t, sizeof(char)* (channels*height*width), cudaMemcpyDeviceToHost);

    write_png(argv[2], host_result_t, height, width, channels);
